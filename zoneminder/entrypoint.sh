...
if [ -n "$ZM_DB_USER" ] && [ -n "$ZM_DB_PASS" ] && [ -n "$ZM_DB_NAME" ] && [ -n "$ZM_DB_HOST" ]; then
    sed -i -e "s/ZM_DB_NAME=zm/ZM_DB_NAME=$ZM_DB_NAME/g" $ZMCONF
    sed -i -e "s/ZM_DB_USER=zmuser/ZM_DB_USER=$ZM_DB_USER/g" $ZMCONF
    sed -i -e "s/ZM_DB_PASS=zmpass/ZM_DB_PASS=$ZM_DB_PASS/g" $ZMCONF
    ...
    if [ -n "$MYSQL_SERVER" ] && [ -n "$MYSQL_USER" ] && [ -n "$MYSQL_PASSWORD" ] && [ -n "$MYSQL_DB" ]; then
        ...
        sed -i -e "s/ZM_DB_PASS=zmpass/ZM_DB_PASS=$MYSQL_PASS/g" $ZMCONF
